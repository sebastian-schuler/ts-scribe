...
import { arrayShuffle } from '../../src/array/index.js';

describe('shuffleArray', () => {
	it('should return an array with the same length after shuffling', () => {
		const inputArray = [1, 2, 3, 4, 5];
		const shuffledArray = arrayShuffle(inputArray);
	...
	it('should shuffle the array', () => {
		const inputArray = [...Array.from({ length: 500 }).keys()];
		const shuffled = arrayShuffle(inputArray);
