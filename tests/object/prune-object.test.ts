...
describe('objectPrune', () => {
	describe('flat objects', () => {
		...
		it('should keep null values', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ a: null, b: undefined })).toEqual({ a: null });
		});
		...
		it('should keep Infinity and -Infinity', () => {
			expect(objectPrune({ a: Number.POSITIVE_INFINITY, b: Number.NEGATIVE_INFINITY })).toEqual({
		...
		it('should keep -0', () => {
			expect(Object.is((objectPrune({ a: -0 }) as { a: number }).a, -0)).toBe(true);
		});

		it('should return an empty object if all values are undefined', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ a: undefined, b: undefined })).toEqual({});
		});

		it('should return the same object shape when no values are undefined', () => {
			expect(objectPrune({ a: 1, b: 'foo', c: true })).toEqual({ a: 1, b: 'foo', c: true });
		});

		it('should return an empty object for an empty input', () => {
			expect(objectPrune({})).toEqual({});
		});
	});
	...
	describe('nested objects', () => {
		...
		it('should keep nested objects that become empty after pruning', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ a: { b: undefined } })).toEqual({ a: {} });
		});
	...
	describe('arrays', () => {
		it('should remove undefined values from a flat array', () => {
			expect(objectPrune([1, undefined, 2, undefined, 3])).toEqual([1, 2, 3]);
		});

		it('should return an empty array if all elements are undefined', () => {
			expect(objectPrune([undefined, undefined])).toEqual([]);
		});

		it('should keep null values in arrays', () => {
			expect(objectPrune([1, null, 2])).toEqual([1, null, 2]);
		});

		it('should keep falsy non-undefined values in arrays', () => {
			expect(objectPrune([0, false, '', Number.NaN, null])).toEqual([0, false, '', Number.NaN, null]);
		});

		it('should recursively compact nested arrays', () => {
			expect(objectPrune([[undefined, 1], [2, undefined]])).toEqual([[1], [2]]);
		});

		it('should keep nested arrays that become empty after pruning', () => {
			expect(objectPrune([[undefined, undefined], [1]])).toEqual([[], [1]]);
		});

		it('should handle sparse arrays by treating holes as undefined', () => {
			// eslint-disable-next-line no-sparse-arrays
			expect(objectPrune([1, , 3])).toEqual([1, 3]);
		});

		it('should return all elements unchanged when none are undefined', () => {
			expect(objectPrune([1, null, false, '', 0])).toEqual([1, null, false, '', 0]);
		});
	...
	describe('mixed objects and arrays', () => {
		it('should recursively compact objects inside arrays', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune([{ a: undefined, b: 1 }, null])).toEqual([{ b: 1 }, null]);
		});

		it('should recursively compact arrays inside objects', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ a: [1, undefined, 2], b: undefined })).toEqual({ a: [1, 2] });
		});
	...
	describe('special values', () => {
		...
		it('should not include inherited enumerable properties', () => {
			...
			expect(objectPrune(input)).toEqual({ own: 1 });
		});
		...
		it('should handle class instances by treating them as plain objects', () => {
			...
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune(new Point(1))).toEqual({ x: 1 });
		});

		it('should treat Map instances as plain objects (becomes {})', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ m: new Map([['a', 1]]) })).toEqual({ m: {} });
		});

		it('should treat Set instances as plain objects (becomes {})', () => {
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ s: new Set([1, 2]) })).toEqual({ s: {} });
		});

		it('should preserve Date instances inside arrays', () => {
			const date = new Date('2026-01-01');
			expect(objectPrune([date, undefined, 1])).toEqual([date, 1]);
		});

		it('should preserve RegExp instances inside arrays', () => {
			const re = /foo/;
			expect(objectPrune([re, undefined, 1])).toEqual([re, 1]);
		});

		it('should preserve Date alongside sibling undefined in nested object', () => {
			const date = new Date('2026-01-01');
			// @ts-expect-error testing should resolve in an error
			expect(objectPrune({ a: { d: date, x: undefined } })).toEqual({ a: { d: date } });
		});
	});

	describe('top-level primitives', () => {
		it('should return null as-is', () => {
			expect(objectPrune(null)).toBe(null);
		});

		it('should return undefined as-is', () => {
			expect(objectPrune(undefined)).toBe(undefined);
		});

		it('should return a number as-is', () => {
			expect(objectPrune(42)).toBe(42);
		});

		it('should return a string as-is', () => {
			expect(objectPrune('hello')).toBe('hello');
		});

		it('should return a boolean as-is', () => {
			expect(objectPrune(false)).toBe(false);
		});

		it('should return a bigint as-is', () => {
			expect(objectPrune(42n)).toBe(42n);
		});

		it('should return a symbol as-is', () => {
			const sym = Symbol('test');
			expect(objectPrune(sym)).toBe(sym);
		});
	});
	...
	describe('immutability', () => {
		...
		it('should return a new object reference, not the original', () => {
			const input = { a: 1 };
			expect(objectPrune(input)).not.toBe(input);
		});
	...
	describe('options', () => {
		describe('deep', () => {
			it('should not recurse into nested objects when deep: false', () => {
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ a: { b: undefined }, c: undefined }, { deep: false })).toEqual({ a: { b: undefined } });
			});

			it('should not recurse into nested arrays when deep: false', () => {
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ a: [undefined, 1], b: undefined }, { deep: false })).toEqual({ a: [undefined, 1] });
			});

			it('should still filter top-level array items when deep: false', () => {
				expect(objectPrune([undefined, 1, undefined, { a: undefined }], { deep: false })).toEqual([1, { a: undefined }]);
			});
			...
			it('should do nothing extra with deep: false and arrays: false', () => {
				...
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ a: [undefined, 1], b: { c: undefined }, d: undefined }, { deep: false, arrays: false })).toEqual({ a: [undefined, 1], b: { c: undefined } });
			});
		});
		...
		describe('arrays', () => {
			it('should return arrays as-is when arrays: false', () => {
				const input = [1, undefined, 2];
				expect(objectPrune(input, { arrays: false })).toBe(input);
			});

			it('should leave object-valued array properties untouched when arrays: false', () => {
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ a: [undefined, 1], b: undefined }, { arrays: false })).toEqual({ a: [undefined, 1] });
			});

			it('should still prune nested objects when arrays: false and deep: true', () => {
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ a: { b: undefined, c: 1 }, d: [undefined, 2] }, { arrays: false })).toEqual({ a: { c: 1 }, d: [undefined, 2] });
			});

			it('should leave nested arrays untouched when arrays: false', () => {
				expect(objectPrune([[undefined, 1], [2, undefined]], { arrays: false })).toEqual([[undefined, 1], [2, undefined]]);
			});
		});
		...
		describe('preserveDate', () => {
			it('should preserve Date instances by default', () => {
				const date = new Date('2026-01-01');
				const result = objectPrune({ d: date, x: undefined });
				expect((result as { d: Date }).d).toBe(date);
			});

			it('should treat Date as a plain object when preserveDate: false', () => {
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ d: new Date() }, { preserveDate: false })).toEqual({ d: {} });
			});
		});
		...
		describe('preserveRegExp', () => {
			...
			it('should treat RegExp as a plain object when preserveRegExp: false', () => {
				// @ts-expect-error testing should resolve in an error
				expect(objectPrune({ re: /foo/ }, { preserveRegExp: false })).toEqual({ re: {} });
			});
		});
	});
});
